...
class TelaFormularioDivida(tk.Toplevel):
    def __init__(self, parent, controlador, moradores: list[Morador], divida_existente: Divida = None):
        ...
        self.title("Editar Dívida" if divida_existente else "Adicionar Nova Dívida")
        ...
        if divida_existente:
            self.desc_var.set(divida_existente.descricao)
            self.valor_var.set(f"{divida_existente.valor:.2f}".replace('.', ','))
            try:
                data_db = datetime.strptime(divida_existente.data_vencimento, '%Y-%m-%d')
                self.vencimento_var.set(data_db.strftime('%d/%m/%Y'))
    ...
    def _criar_formulario(self):
        ...
        ttk.Label(frame, text="Data de Vencimento (DD/MM/AAAA):", font=('Arial', 10, 'bold')).pack(anchor="w")
    ...
    def _salvar(self):
        try:
            ...
            if self.controlador.salvar_divida(dados):
                self.destroy()
